...
static EAGLView* sharedEAGLView = nil;

//variables for quiting
static CGPoint initialPoint;
static CGPoint finalPoint;
static BOOL abortIsStillPossible;//this one is useful to avoid some cases doing an abort without wanting to do one
...
@synthesize mouseMotionFunction;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for(UITouch *touch in allTouches) {
        if ([touch phase]==UITouchPhaseBegan) {
            ...
            if ([allTouches count]==1) {
                initialPoint = pt;
                //printf("x = %f; y = %f\n",pt.x,pt.y);
                abortIsStillPossible=YES;
            } 
            else abortIsStillPossible=NO;
    ...
    //Considère a touch as a mouse click push
    if (!handled && self.mouseFunction) {
        self.mouseFunction(WS_LEFT_BUTTON, WS_MOUSE_DOWN, pt.x - self.bounds.origin.x, pt.y - self.bounds.origin.y);
    }
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event;
{
    ...
    for(UITouch *touch in allTouches) {
        ...
        if ([touch phase] == UITouchPhaseMoved) {
            if(self.mouseMotionFunction) {
                self.mouseMotionFunction(pt.x - self.bounds.origin.x, pt.y - self.bounds.origin.y);
            }
        }
    }
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event { 
    ...
    for(UITouch *touch in allTouches) {
        ...
        if ([allTouches count]==1 && abortIsStillPossible) {
    ...
    if (!handled && self.mouseFunction) {
        self.mouseFunction(WS_LEFT_BUTTON, WS_MOUSE_UP, pt.x - self.bounds.origin.x, pt.y - self.bounds.origin.y);
    }
